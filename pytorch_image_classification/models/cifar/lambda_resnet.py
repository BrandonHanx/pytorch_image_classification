...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.conv2 = LambdaConvLayer(bottleneck_channels, stride=stride)
...
class Network(nn.Module):
    def __init__(self, config):
        ...
        self.stage1 = self._make_stage(
            n_channels[0], n_channels[0], n_blocks[0], block, stride=1
        )
        self.stage2 = self._make_stage(
            n_channels[0], n_channels[1], n_blocks[1], block, stride=2
        )
        self.stage3 = self._make_stage(
            n_channels[1], n_channels[2], n_blocks[2], block, stride=2
        )
        self.stage4 = self._make_stage(
            n_channels[2], n_channels[3], n_blocks[3], block, stride=2
        )
